...
## Installing NixOS onto NVMe

This flake provides modules for building an live USB / SD card image that installs a
NixOS system onto an NVMe drive attached to a Raspberry Pi 5.

### Modules

- `nixosModules.nvme-target` — NVMe target system configuration (filesystem
  layout, kernel params, firmware partition generation)
- `nixosModules.nvme-installer` — SD card installer (partitions NVMe,
  copies closure, configures EEPROM)

### Usage

Define two system configurations: the **target** (what runs on NVMe) and
the **installer** (the SD card image that installs it):

```nix
{
  nixosConfigurations = {
    # The system that will be installed onto NVMe
    my-nvme-target = nixpkgs.lib.nixosSystem {
      system = "aarch64-linux";
      modules = [
        raspberry-pi-nix.nixosModules.raspberry-pi
        raspberry-pi-nix.nixosModules.nvme-target
        {
    ...
    # The SD card installer image
    my-nvme-installer = nixpkgs.lib.nixosSystem {
      system = "aarch64-linux";
      modules = [
        raspberry-pi-nix.nixosModules.raspberry-pi
        raspberry-pi-nix.nixosModules.sd-image
        raspberry-pi-nix.nixosModules.nvme-installer
        ({ lib, ... }: {
          raspberry-pi-nix.board = self.nixosConfigurations.my-nvme-target.config.raspberry-pi-nix.board;
          raspberry-pi-nix.kernel-version = self.nixosConfigurations.my-nvme-target.config.raspberry-pi-nix.kernel-version;
          networking.hostName = "installer";
          networking.useDHCP = true;
          nvme-installer = {
            enable = true;
            targetSystem = self.nixosConfigurations.my-nvme-target;
          };
        })
      ];
    };
...
```
nix build '.#nixosConfigurations.my-nvme-installer.config.system.build.sdImage'
```

### Flashing and running

```bash
zstdcat result/sd-image/*.img.zst | sudo dd of=/dev/sdX bs=4M status=progress iflag=fullblock
```

Boot the Pi from the SD card. It will auto-login as root and display
instructions. Run `install-nvme` to partition the NVMe, copy the system
closure, and configure the EEPROM. The installer enables SSH (root/nixos) and
sets a static IP on `end0` (192.168.1.100) for headless access.
After installation:

1. Power off the Pi
2. Remove the SD card
3. The Pi boots from NVMe (default boot order: SD, USB, NVMe)
